import { BUILD_COMPLETE, DO_UPDATE, DONE_UPDATE, LOCAL_RELOAD_SOCKET_PORT, LOCAL_RELOAD_SOCKET_URL, } from '../consts.js';
    ...
    wss.on('connection', ws => {
        ...
        ws.addEventListener('message', event => {
            ...
            if (message.type === BUILD_COMPLETE) {
                clientsThatNeedToUpdate.forEach((ws) => ws.send(MessageInterpreter.send({ type: DO_UPDATE, id: message.id })));
            }
        });
    });
    wss.on('error', (error) => {
