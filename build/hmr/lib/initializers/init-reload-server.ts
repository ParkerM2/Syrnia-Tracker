  ...
  ws.addEventListener("message", event => {
    ...
    if (message.type === BUILD_COMPLETE) {
      clientsThatNeedToUpdate.forEach((client: WebSocket) =>
        client.send(MessageInterpreter.send({ type: DO_UPDATE, id: message.id })),
      );
    }
  });
});
...
/** Broadcast a reload signal to all connected extension clients */
export const broadcastReload = (id: string): void => {
  clientsThatNeedToUpdate.forEach((client: WebSocket) => client.send(MessageInterpreter.send({ type: DO_UPDATE, id })));
};
