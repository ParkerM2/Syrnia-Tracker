import { UPDATE_USER_STATS } from '@extension/shared';
import { scrapeUserStats } from '@src/scrapeUserStats';
    ...
    if (userStats && userStats.username && Object.keys(userStats.skills).length > 0) {
      ...
      chrome.runtime.sendMessage({ type: UPDATE_USER_STATS, data: userStats }, () => {
        if (chrome.runtime.lastError) {
          console.error('[Stats Page] Error sending message:', chrome.runtime.lastError);
        }
      });
    }
