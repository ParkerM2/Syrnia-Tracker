...
import { cn, Card, CardContent, CardHeader, CardDescription } from '@extension/ui';
import { TrendDownIcon, TrendUpIcon } from '@src/assets/icons';
...
const MetricCard = memo(({ title, value, subtitle, trend, lootItems }: MetricCardProps) => (
  <Card className="pt-2">
          ...
          <div className="border-foreground/20 dark:border-foreground/40 flex flex-col items-end gap-1 rounded-md border bg-green-500/20 px-2 py-1 dark:bg-teal-500/20">
            <div
              ...
              {trend.isPositive ? (
                ...
                <TrendDownIcon className="text-foreground h-4 w-4" />
              )}
              {trend.isPositive ? '+' : ''}
              {trend.value}%
            </div>
    ...
    <p className="text-muted-foreground pb-2 text-sm">{subtitle}</p>
      ...
      <div className={cn('flex flex-col gap-4')}>
        <Card>
          <CardContent className="p-8 text-center">
            <p className="text-muted-foreground text-sm">No tracked data found.</p>
