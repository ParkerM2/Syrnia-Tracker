...
import { formatLocation } from '@extension/shared';
import {
  cn,
  Card,
  CardContent,
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex flex-col gap-2">
            <label htmlFor="monster-filter" className="text-muted-foreground text-sm font-medium">
              Filter by Monster
            </label>
            <select
              id="monster-filter"
              ...
              className={cn(
                'border-input text-foreground ring-offset-background focus-visible:ring-ring bg-background flex h-10 w-full rounded-md border px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-[hsl(var(--secondary))]',
              )}>
            ...
            <div className="flex flex-col gap-6 sm:flex-row">
              ...
              <div className="flex flex-1 flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col">
                    <span className="text-muted-foreground text-sm">Total Fights Tracked</span>
                    <span className="text-2xl font-bold">{locationStats.totalFights.toLocaleString()}</span>
                  </div>
                  {locationStats.avgHitsToKill > 0 && (
                    <div className="flex flex-col">
                      <span className="text-muted-foreground text-sm">Avg Hits to Kill</span>
                      <span className="text-2xl font-bold">{locationStats.avgHitsToKill.toFixed(1)}</span>
                    </div>
                  )}
                  <div className="flex flex-col">
                    <span className="text-muted-foreground text-sm">Avg Damage per 15 Min</span>
                    <span className="text-2xl font-bold text-red-500">
                      {Math.round(locationStats.avgDamagePer15Min).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-muted-foreground text-sm">Max Damage per 15 Min</span>
                    <span className="text-2xl font-bold text-red-500">
